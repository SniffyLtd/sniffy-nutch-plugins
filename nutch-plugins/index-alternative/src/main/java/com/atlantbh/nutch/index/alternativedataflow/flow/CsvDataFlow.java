...
public class CsvDataFlow implements DataFlow {
    ...
    // Internal data
    private final Map<String, CSVPrint> nameCsvPrintMap = new HashMap<String, CSVPrint>();
    ...
    private void initWriters() throws IOException {
        ...
        if (!initialized) {
            ...
            for (Entry entry : entryList) {
                if ("CSV".equals(entry.getDataFlow())) {
                    ...
                    if (Boolean.valueOf(FilterUtils.getNullSafe(entry.getParameterMap().get(APPEND_TIMESTAMP), "false"))) {
                        csvFile = new File(entry.getParameterMap().get(DESTINATION) + File.separator
                                + entry.getParameterMap().get(NAME) + timestamp + ".csv");
                    } else {
                        csvFile = new File(entry.getParameterMap().get(DESTINATION) + File.separator
                                + entry.getParameterMap().get(NAME) + ".csv");
                    }
                    ...
                    if ("UNIX".equals(style)) {
                        csvPrint = new CSVPrinter(new OutputStreamWriter(new FileOutputStream(csvFile), "UTF-8"), '#',
                                quoteCharacter, seperator, false, true);
                    } else if ("EXCEL".equals(style)) {
                        csvPrint = new ExcelCSVPrinter(new OutputStreamWriter(new FileOutputStream(csvFile), "UTF-8"),
                                quoteCharacter, seperator, false, true);
                    }
                    nameCsvPrintMap.put(entry.getParameterMap().get(NAME), csvPrint);

                    // If this flag is set add the first
                    if (Boolean.valueOf(FilterUtils.getNullSafe(entry.getParameterMap().get(ADD_FIELD_NAMES_TO_FIRST_LINE),
                            "false"))) {
